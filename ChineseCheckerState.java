...
public class ChineseCheckerState extends BoardState {
	...
	protected Point2D player1SD;
	protected Point2D player2SD;
	...
	ChineseCheckerState(ChineseCheckerState state) {
		...
		this.player1SD = state.player1SD;
		this.player2SD = state.player2SD;
	...
	private void createInitialBoard() {
		...
		player1SD = calculateStandardDeviation(Player.One);
		player2SD = calculateStandardDeviation(Player.Two);
	...
	public Point2D calculateStandardDeviation(Player player) {
		int count = 0;

		Point2D mean = calculateCenterOfGravity(player);
		Point2D sumOfSquares = new Point2D();

		for (int y = 0; y < boardSize; y++) {
			for (int x = 0; x < boardSize; x++) {
				if (boardCells[y][x] == player) {
					count++;
					Point2D diff = new Point2D(x - mean.x, y - mean.y);
					sumOfSquares.x += diff.x * diff.x;
					sumOfSquares.y += diff.y * diff.y;
				}
			}
		}

		return new Point2D(Math.sqrt(sumOfSquares.x / count), Math.sqrt(sumOfSquares.y / count));
	}
